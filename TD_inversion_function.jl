...
function TD_inversion_function(
    ...
    if length(model_checkpoint_lists) == 0
        @time while valid == 0
            (model, dataStruct, valid) = build_starting(TD_parameters, dataStruct, RayTraces)
        end
        iter_ind        = 1
        model_hist      = []
        cellnumber_list = []
        ...
        println("----Loading Chain" * string(chain) *"------")
        @time load_model  = load(model_checkpoint_lists[model_ind])
        # yurong 11/27/22 in this case, 5% is larger than 45%
    ...
    for iter in iter_ind:TD_parameters["n_iter"]
            ...
            if model.nCells < TD_parameters["max_cells"]
                ...
                if TD_parameters["prior"] == 1 # Uniform
                    if zetanew > 0 && zetanew < TD_parameters["zeta_scale"]
                        (modeln, dataStructn, valid) = evaluate(modeln, dataStruct, RayTraces, TD_parameters)
        ...
        elseif action == 5 # change sigma
            ...
            if sig_n > 0 && sig_n < TD_parameters["max_sig"]
                ...
                (modeln, dataStructn, valid) = evaluate(model, dataStructn, RayTraces, TD_parameters)
        ...
        if TD_parameters["plot_voronoi"] == true
            ENV["GKSwstype"] = "nul"
            plot_voronoi(model, dataStruct, TD_parameters, chain, iter)
        end
        ...
        if iter >= TD_parameters["burn_in"]
            ...
            if -1e-9 < mod(iter*100/TD_parameters["n_iter"],TD_parameters["save_percent"]) < 1e-9
                CurrentModel = model
                percentage = Int(100 * iter / TD_parameters["n_iter"])
                modelname = "./models/chain" * string(chain) * "_iter" * string(Int(iter)) * "_" * string(percentage)* "%.jld"
                println("----Saving Chain" * string(chain) * " "* string(Int(100 * iter / TD_parameters["n_iter"]))* "% models------")
                @time save(modelname,"model",CurrentModel,"dataStruct",dataStruct,"iter",iter,"saved_#",used_num,"model_num",model_num,"model_hist",model_hist,"burnin",true,"nCells",cellnumber_list,"phi",phi_list)
                delete_old_checkpoint(chain, iter, percentage, TD_parameters["save_percent"])
                CurrentModel = nothing
            end
            
        elseif -1e-9 < mod(iter*100/TD_parameters["n_iter"],TD_parameters["save_percent"]) < 1e-9
            CurrentModel = model
            percentage = Int(100 * iter / TD_parameters["n_iter"])
            modelname = "./models/chain" * string(chain) * "_iter" * string(Int(iter)) * "_" * string(percentage)* "%.jld"
            println("----Saving Chain" * string(chain) * " "* string(Int(100 * iter / TD_parameters["n_iter"]))* "% models------")
